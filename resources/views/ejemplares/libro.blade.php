...
<body>
    ...
    @if(!empty($ejemplar->contenido) && (Str::endsWith($ejemplar->contenido, '.pdf') || Str::startsWith($ejemplar->contenido, 'data:application/pdf')))
        <div class="libro__pdf">
            <canvas id="pdf"></canvas>
        </div>
    @elseif(!empty($ejemplar->contenido))
        <div class="libro__contenido">
            {!! $ejemplar->contenido !!}
        </div>
    @else
        <div class="libro__contenido--empty">
            <p>{{ __('No content available for this book.') }}</p>
        </div>
    @endif
